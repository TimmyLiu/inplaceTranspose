...
{
   const int numGroupsY_1 = 528;
   size_t g_index;
   
   size_t iOffset = 0;
   g_index = get_group_id(0);
   iOffset += (g_index/numGroupsY_1)*1048576;//1048576
   ...
   const int idx = lidx + (t_gx_p<<5);//*32
   const int idy = lidy + (t_gy_p<<5);
   
   const int starting_index_yx = (t_gy_p<<5)+ (t_gx_p<<15);//32768
   ...
   for (int loop = 0; loop<4; ++loop){
      ...
      tmpm = inputA[(idy + loop *8)*1024 + idx];
      tmpt = inputA[(lidy + loop *8)*1024 + lidx + starting_index_yx];
      xy_s[index] = tmpm; 
      yx_s[index] = tmpt; 
   }
   ...
   for (int loop = 0; loop<4; ++loop){
      ...
      outputA[(idy + loop*8)*1024 + idx] = yx_s[index];
      outputA[(lidy + loop*8)*1024 + lidx+ starting_index_yx] = xy_s[index];
   }
}
