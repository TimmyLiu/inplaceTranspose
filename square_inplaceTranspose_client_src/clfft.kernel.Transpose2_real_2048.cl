...
{
   const int numGroupsY_1 = 2080; //(dim / 32) * (dim / 32 + 1) / 2
   size_t g_index;
   
   size_t iOffset = 0;
   g_index = get_group_id(0);
   iOffset += (g_index/numGroupsY_1)*4194304; // dim * dim
   ...
   //-129 = -2*m-1
   //16640 = 4*m*(m+1)
   float row = (-129+sqrt((16640-8.0f*g_index- 7)))/ (-2.0f);
   if (row == (float)(int)row) row -= 1; 
   const int t_gy = (int)row;
   
   const int t_gx_p = g_index - 64*t_gy + t_gy*(t_gy + 1) / 2;
   ...
   const int starting_index_yx = t_gy_p*32 + t_gx_p*65536;//32 * dim
