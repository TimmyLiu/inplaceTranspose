...
{
   const int numGroupsY_1 = 8128; //(dim / 32) * (dim / 32 + 1) / 2
   size_t g_index;
   ...
   //-255 = -2*m-1
   //65024 = 4*m*(m+1)
   float row = (-255+sqrt((65024-8.0f*g_index- 7)))/ (-2.0f);
   if (row == (float)(int)row) row -= 1; 
   const int t_gy = (int)row;
   
   const int t_gx_p = g_index - 127*t_gy + t_gy*(t_gy + 1) / 2;//m
   ...
   const int starting_index_yx = t_gy_p*32 + t_gx_p*130048;//32 * dim
   
   __local float xy_s[1024];
   __local float yx_s[1024];
   float tmpm, tmpt;
   
   int index;
   for (int loop = 0; loop<4; ++loop){
      index = lidy*32 + lidx + loop*256;
      tmpm = inputA[(idy + loop *8)*4064 + idx];
      tmpt = inputA[(lidy + loop *8)*4064 + lidx + starting_index_yx];
      xy_s[index] = tmpm; 
      yx_s[index] = tmpt; 
   }
   ...
   for (int loop = 0; loop<4; ++loop){
      ...
      outputA[(idy + loop*8)*4064 + idx] = yx_s[index];
      outputA[(lidy + loop*8)*4064 + lidx+ starting_index_yx] = xy_s[index];
   }
}
